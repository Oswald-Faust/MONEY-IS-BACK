              ...
              <div className="flex-1 min-w-0 pr-2">
                <div className="relative group/ws">
                  <button 
                    ...
                    className="w-full flex items-center gap-3 p-1 rounded-xl hover:bg-glass-hover transition-all text-left"
                  >
                    <div 
                      className="w-9 h-9 rounded-xl flex items-center justify-center relative overflow-hidden flex-shrink-0 shadow-sm"
                      style={{ 
                  ...
                  <div id="ws-dropdown" className="hidden absolute top-full left-0 w-64 mt-2 py-2 bg-bg-card border border-glass-border rounded-2xl shadow-2xl z-[100] backdrop-blur-xl">
                      ...
                      {workspaces.map((ws) => (
                        <button
                          ...
                          className={`w-full flex items-center gap-3 p-2 rounded-xl transition-all ${
                            currentWorkspace?._id === ws._id ? 'bg-indigo-500/10 border border-indigo-500/20' : 'hover:bg-glass-hover border border-transparent'
                          }`}
                        >
                          <div 
                          ...
                          <div className="flex-1 text-left min-w-0">
                            <p className={`text-sm font-medium truncate ${currentWorkspace?._id === ws._id ? 'text-indigo-400' : 'text-text-main'}`}>
                              {ws.name}
                            </p>
                            <p className="text-[10px] text-text-dim">{ws.subscriptionPlan.toUpperCase()}</p>
                          </div>
                          ...
                          {currentWorkspace?._id === ws._id && (
                            <Link 
                                ...
                                className="p-1.5 rounded-lg hover:bg-glass-hover text-text-dim hover:text-text-main transition-all mr-1"
                            >
                    ...
                    <div className="px-2 mt-2 pt-2 border-t border-glass-border/50">
                      {workspaces.some(ws => ['team', 'enterprise'].includes(ws.subscriptionPlan)) || user?.role === 'admin' || workspaces.length < 1 ? (
                        <button
                          ...
                          className="w-full flex items-center gap-3 p-2 rounded-xl text-text-dim hover:bg-glass-hover hover:text-text-main transition-all group"
                        >
                        ...
                        <Link
                          ...
                          className="w-full flex items-center gap-3 p-2 rounded-xl text-text-dim hover:bg-glass-hover hover:text-indigo-400 transition-all group"
                        >
                          <div className="w-8 h-8 rounded-lg border border-dashed border-glass-border flex items-center justify-center group-hover:border-indigo-500/50 group-hover:bg-indigo-500/10 transition-all">
      ...
      <div className="p-3">
        ...
        `}>
          ...
          {(!sidebarCollapsed || isMobile) && (
            <div className="flex items-center gap-1">
              <Link href="/settings">
                <button className="p-1.5 rounded-lg hover:bg-indigo-500/10 text-text-dim hover:text-indigo-400 transition-colors">
