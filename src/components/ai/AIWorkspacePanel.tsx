...
export default function AIWorkspacePanel({ variant = 'page', onClose }: AIWorkspacePanelProps) {
  ...
  const handleNewConversation = async () => {
    try {
      ...
      if (conv) { setSelectedConversationId(conv._id); setMessages([]); }
    } catch {
  ...
  // ─── Page variant ──────────────────────────────────────────────────────────
  return (
    <div className="grid min-h-[calc(100vh-6rem)] grid-cols-1 overflow-hidden rounded-[28px] border border-glass-border bg-bg-secondary/60 xl:grid-cols-[280px_minmax(0,1fr)]">

      {/* ── Sidebar ── */}
      <aside className="flex flex-col border-b border-glass-border xl:border-b-0 xl:border-r">
        {/* Sidebar header */}
                  ...
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline justify-between gap-1">
                        <p className="text-sm font-semibold text-text-main truncate leading-tight">{conv.title}</p>
        ...
        {/* Sidebar footer — quick suggestions */}
        <div className="p-3 border-t border-glass-border">
          ...
          <div className="space-y-1">
            {quickSuggestions.map((s) => (
              <button
                key={s}
                onClick={() => { setComposer(s); textareaRef.current?.focus(); }}
      ...
      <div className="flex flex-col min-h-0">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-glass-border flex-shrink-0">
          <div className="flex items-center gap-3">
            <div className="relative w-10 h-10 flex-shrink-0">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-indigo-500/30 via-violet-500/20 to-cyan-500/20 flex items-center justify-center">
                <Sparkles className="w-5 h-5 text-indigo-300" />
              </div>
              <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-emerald-500 rounded-full border-2 border-bg-secondary" />
            </div>
            <div>
              <p className="text-base font-bold text-text-main leading-tight">{t.aiPage.floatingTitle}</p>
              <p className="text-xs text-emerald-400 font-medium mt-0.5">En ligne · {t.aiPage.floatingSubtitle}</p>
            </div>
          </div>
          <button
            onClick={handleNewConversation}
            className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-xs font-semibold text-text-main transition-colors hover:bg-glass-hover"
          >
          ...
          {isLoadingMessages ? (
            ...
            <div className="space-y-5 max-w-3xl mx-auto">
              <AnimatePresence initial={false}>
                {messages.map((msg) => {
                  ...
                  return (
                    ...
                    >
                      {isAssistant && (
                        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500/30 to-cyan-500/20 flex items-center justify-center flex-shrink-0 mt-1">
                          <Sparkles className="w-4 h-4 text-indigo-300" />
                        </div>
                      )}
                      <div className={`max-w-[78%] space-y-2 ${isAssistant ? '' : 'items-end flex flex-col'}`}>
                        <div
              ...
              {isSending && (
                ...
                >
                  <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-indigo-500/30 to-cyan-500/20 flex items-center justify-center flex-shrink-0">
                    <Sparkles className="w-4 h-4 text-indigo-300" />
                  </div>
                  <div className="rounded-[20px] rounded-tl-sm bg-white/[0.05] border border-white/[0.08] px-4 py-3">
        ...
        {/* Composer */}
        <div className="flex-shrink-0 border-t border-glass-border px-6 py-4">
          <form onSubmit={handleSend} className="flex items-end gap-3">
            <textarea
              ...
              rows={1}
              placeholder={t.aiPage.composerPlaceholder}
              className="flex-1 resize-none rounded-2xl border border-glass-border bg-bg-tertiary px-4 py-3.5 text-sm text-text-main placeholder:text-text-muted focus:border-accent-primary/40 focus:outline-none focus:ring-4 focus:ring-accent-primary/5 min-h-[52px] max-h-[160px]"
            />
            <button
              type="submit"
              disabled={!composer.trim() || isSending}
              className="h-[52px] w-[52px] flex-shrink-0 flex items-center justify-center rounded-2xl bg-accent-primary text-white shadow-lg shadow-accent-primary/20 transition-all hover:opacity-90 disabled:opacity-40"
            >
              {isSending ? <Loader2 className="w-5 h-5 animate-spin" /> : <Send className="w-5 h-5" />}
            </button>
          </form>
          <p className="mt-1.5 text-[10px] text-text-muted text-right">⌘ + Entrée pour envoyer</p>
        </div>
      </div>
    </div>
  );
